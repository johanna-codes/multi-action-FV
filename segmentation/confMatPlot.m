...
switch(lower(opt.mode))
	...
	case lower('percentage')
		%opt.matPlotOpt.matrixName=sprintf('Percentages, RR = %g%%',
		%newOverall);%Joha
		opt.matPlotOpt.showColSum=0;
        opt.matPlotOpt.showRowRightLabel=0; %Joha
        opt.matPlotOpt.showColDownLabel=0; %Joha
...
% 		for j=1:n
% 			str = get(obj.colDownLabel(j), 'string');
% 			set(obj.colDownLabel(j), 'string', [str, '%']); %Joha
% 			%commented this
% 		end
	case lower('both')
