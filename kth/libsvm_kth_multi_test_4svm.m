...
for d = 1:4
    fprintf('Training one SVM per scenario %d\n', d);
    sc = int2str(d);
    data_train = [];
    labels_train = [];
    data_test = [];
...
%display('Training for scenario...', sc);
model = svmtrain(labels_train, data_train, ['-s 0 -t 0 -b 1' ]);
save_name = strcat('./run', run,  '/svm_model_sc', sc);
...
L =13;
n_sc = 4; %Total # of scenarios
ACC = [];

test_info =  cell(n_sc*n_pe_te,4);
%Loading 4 SVM models for KTH
display('Loading SVM Models'); % loading 'model'
kth_models=  cell(n_sc,1);


for d = 1:n_sc
    sc = int2str(d);
    load_name = strcat('./run', run,  '/svm_model_sc', sc);
    sLoad= char(load_name);
    kth_models{d} = load(sLoad);
...
for d = 1:n_sc
    ...
    for pe = 1:n_pe_te
       ...
       while(more) % 
           ...
           [pre_label pre_prob_label] = classify_segment_kth(ini, fin, fr_idx_2, feat_video, w, mu, sigma, kth_models);
            
           fin = fin + 1; 
           [label prob_label] = classify_segment_kth(ini, fin, fr_idx_2, feat_video, w, mu, sigma, kth_models);
...
save_info = strcat('./run', run, '/test_info_4svm.mat');
